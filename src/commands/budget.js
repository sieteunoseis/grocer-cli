...
budgetCmd
  ...
  .action(() => {
    ...
    if (!budget) {
      ...
      console.log(chalk.dim("Set one: grocer budget set 150 --period weekly"));
      return;
    }
    ...
    console.log(`  Spent:     $${spending.spent.toFixed(2)} (${spending.trips} trips)`);
...
function getCurrentPeriod(budget) {
  ...
  const daysSinceStart = Math.floor((today - start) / (1000 * 60 * 60 * 24));
  const currentPeriodIndex = Math.max(0, Math.floor(daysSinceStart / periodDays));
